...
let consonants = ['b', 'c', 'd', 'f', 'g', 'h', 'k', 'l',
 'm', 'n', 'p', 'r', 's', 't', 'v', 'x', 'y'];
let vpairs = ['ee', 'oo', 'ea', 'ou', 'oi', 'io', 'au', 'ia'];
...
let fullnametitles = ["Mr. ", "Miss ", "Ms. ", "Mx. ", "Dr. ", "Mme. ", "Sir ", "Lord ", "Lady "];
...
function gen() {
    if(type.value == "full names") {
        let titlechoice = fullnametitles[Math.round(Math.random() * (fullnametitles.length - 1))];
        text.innerHTML = titlechoice + capitalize(genname()) + " " + capitalize(genname());
    }
    else {
