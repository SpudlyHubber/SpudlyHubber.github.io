...
const effects = {
    modify: function() {
        ...
        for(var i = 0; i < data.length; i += 4) {
            ...
            data[i + 2] = Math.round(data[i + 2] + nfilter[0] * 2.55);
            let avg = (data[i] + data[i + 1] + data[i + 2]) / 3;
            let newAvg = Math.floor(Math.floor(avg) / (255 / nfilter[1])) * (255 / nfilter[1]);
            data[i] *= newAvg / avg;
            data[i + 1] *= newAvg / avg;
            data[i + 2] *= newAvg / avg;
        }
...
bright.oninput = function() {
    brightval.innerText = "Brightness: " + bright.value + "%";
    nfilter[0] = parseInt(bright.value);
}
shade.oninput = function() {
    shadeval.innerText = "Number of distinct shades: " + shade.value;
    nfilter[1] = parseInt(shade.value);
}
